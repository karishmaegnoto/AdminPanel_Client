...
export default function AssignUsersModal({ isOpen, onClose, leadId, onSuccess }) {
  ...
  const permissionOptions = ['read', 'edit', 'delete'];
  ...
  const handleAssign = async () => {
    ...
    for (let u of assignedUsers) { if (!u.permissions || u.permissions.length === 0) return alert(`Select at least one permission for ${u.firstName}`); }
    try {
